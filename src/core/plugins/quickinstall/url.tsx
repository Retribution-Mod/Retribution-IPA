...
function handleDeepLink(link: string) {
    if (!link.startsWith("retribution://")) return;
...
export default () => {
    ...
    const linkSub = Linking.addEventListener("url", (event: { url: string }) => handleDeepLink(event.url));
    ...
    return () => {
        patches.forEach(p => p());
        if (typeof linkSub === "function") linkSub();
        else if (linkSub && "remove" in linkSub) (linkSub as any).remove();
    };
};
