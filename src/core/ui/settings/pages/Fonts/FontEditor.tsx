...
function promptDetachConfirmationForThen(fontName: string | undefined, cb: () => void) {
    if (fontName && fonts[fontName].source) openAlert("Retribution-fonts-detach-source-confirmation", <AlertModal
...
function RetributionFontsExtractor({ fonts, setName }: {
            ...
            {!props.name
                ? <TableRowGroup title="Import">
                    ...
                    {getCurrentTheme()?.data?.fonts && <TableRow
                        ...
                        onPress={() => promptActionSheet(RetributionFontsExtractor, fontEntries, { setName })}
                    />}
                    <TableRow
