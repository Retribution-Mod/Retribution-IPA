...
export function useProxy<T>(storage: T): T {
    ...
    React.useEffect(() => {
        const emitter = (storage as any)?.[emitterSymbol] as Emitter | undefined;
        ...
        // Storage is still initializing (e.g. wrapSync not yet resolved);
        // request a re-render once it is ready so the effect re-runs with the emitter.
        const awaitInit = (storage as any)?.[syncAwaitSymbol] as ((cb: () => void) => void) | undefined;
        if (awaitInit) {
            let done = false;
            awaitInit(() => {
                if (done) return;
